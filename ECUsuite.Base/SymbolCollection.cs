...
namespace ECUsuite.ECU.Base
{
    public class SymbolCollection : SortableCollectionBase, ICustomTypeDescriptor
    {
        ...
        {
            // Erstellt eine neue Liste vom Typ SymbolHelper
            List<SymbolHelper> symbolHelperList = new List<SymbolHelper>();

            // Iteriere über alle Elemente in der SymbolCollection und füge sie zur Liste hinzu
            foreach (SymbolHelper symbol in this.List)
            {
                symbolHelperList.Add(symbol);
            }

            // Rückgabe der erstellten Liste
            return symbolHelperList;
        }
