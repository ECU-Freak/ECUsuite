...
namespace ECUsuite.ECU.EDC15
{
    class EDC15P_EEPROM
    {
        ...
        public void LoadFile(string filename)
        {
            ...
            _vin = Encoding.ASCII.GetString(EEPRomBytes, 0x140, 0x11);
            _immo = Encoding.ASCII.GetString(EEPRomBytes, 0x131, 0x0E);
        }
        ...
        public void UpdateFile(string filename)
        {
            ...
            if (Vin.Length == 17)
            {
                byte[] _vinbytes = Encoding.ASCII.GetBytes(_vin);
                int start = 0x140;
            ...
            if (_immo.Length == 14)
            {
                byte[] _immobytes = Encoding.ASCII.GetBytes(_immo);
                int start = 0x131;
