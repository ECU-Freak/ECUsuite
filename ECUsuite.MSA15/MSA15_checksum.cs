...
namespace ECUsuite.ECU.MSA15
{
    public class MSA15_checksum
    {
        ...
        public MSA15_checksum(Tools toolsRef)
        {
            tools = toolsRef;
        }
        ...
        public void DumpChecksumLocations(string info, byte[] allBytes)
        {
            ...
            {
                found = false;
                checksumAddress = tools.findSequence(allBytes, checksumAddress, new byte[2] { b1, b2 }, new byte[2] { 1, 1 });
        ...
        public ChecksumResult tdi41_checksum_search(byte[] file_buffer, uint file_size, bool debug)
        {
            ...
            uint[] chk_array = new uint[7] { 0x00000, 0x000B80, 0x008000, 0x030000, 0x038000, 0x038B80, 0x40000 };
        ...
        uint tdi41_2002_checksum_calculate(byte[] file_buffer, uint chk_start_addr, uint chk_end_addr, ushort seed_a, ushort seed_b, ushort seed_c, ushort seed_d, bool first_pass)
        {
            ...
            if (!first_pass)
            {
                ...
                checksum = (uint)(var_1 + ((var_5 ^ var_2) << 16));
            }
            else
            {
                checksum = (uint)(var_1 + (var_2 << 16));
            }
